...
train = pd.read_csv('../../data/train_splits_removed.csv')

features = pd.read_csv('../../data/features.csv',index_col=0)
features = features.loc[train['index'],:]
...
results = pd.DataFrame(np.concat(all_losses, columns = ["Training Loss", "Validation Loss", "Test Loss"], index = c_values)
