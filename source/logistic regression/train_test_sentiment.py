...
if len(train)==len(features):
	...
	sentiment_features = sentiment_features.iloc[rows_with_embeddings]
else:
...
sentiment_features = np.hstack((np.abs(np.diff(sentiment_features,axis=1)),np.reshape(np.mean(sentiment_features,axis=1),(len(features),1))))
...
add_sentiment = [False, True]
all_losses = np.zeros((len(add_sentiment), 3))
all_auc = np.zeros((len(add_sentiment),3))
for j, add in enumerate(add_sentiment):
	i=0
	loss = np.zeros((5,3))
	auc = np.zeros((5,3))
	if add:
		features = np.hstack([features,sentiment_features])
...
results = pd.DataFrame(all_losses, columns = ["Training Loss", "Validation Loss", "Test Loss"], index = ['no_sentiment','with_sentiment'])

results.to_csv('lr_losses_comparing_sentiments.csv')

x_train = np.hstack([np.abs(s1_emb - s2_emb), features])
model = LogisticRegression(C=.1)
model.fit(x_train,labels)
log_loss(labels, model.predict_proba(x_train))
pickle.dump(model, open('logistic_model_with_sentiment.sav', 'wb'))
